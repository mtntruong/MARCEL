...
class VecLayerNorm(torch.nn.Module):
    ...
    def forward(self, vec: Tensor) -> Tensor:
        ...
        raise ValueError(
            f"'{self.__class__.__name__}' only support 3 or 8 " f"channels (got {vec.size(1)})"
        )
...
class ViS_MP_Vertex(ViS_MP):
    ...
    def __init__(
        ...
        super().__init__(
            num_heads, hidden_channels, cutoff, vecnorm_type, trainable_vecnorm, last_layer
        )
...
class ViSNetBlock(torch.nn.Module):
    ...
    def forward(
        ...
        vec = torch.zeros(
            x.size(0), ((self.lmax + 1) ** 2) - 1, x.size(1), dtype=x.dtype, device=x.device
        )
...
class ViSNet(torch.nn.Module):
    ...
    def __init__(
        ...
        self.output_model = EquivariantScalar(
            hidden_channels=hidden_channels, output_channels=hidden_channels // 2
        )

        self.prior_model = Atomref(atomref=atomref, max_z=max_z)

        # initialize params for barycenter
        # self.representation_model_bary = ViSNetBlock(
        #     lmax=lmax,
        #     vecnorm_type=vecnorm_type,
        #     trainable_vecnorm=trainable_vecnorm,
        #     num_heads=num_heads,
        ...
        self.output_model_bary = EquivariantScalar(
            hidden_channels=hidden_channels, output_channels=hidden_channels // 2
        )

        self.prior_model_bary = Atomref(atomref=atomref, max_z=max_z)
